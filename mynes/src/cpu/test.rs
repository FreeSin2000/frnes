    ...
    fn test_bvs_does_not_branch_when_overflow_clear() {
        ...
        cpu.load(vec![
            ...
            0xa9, 0x00, // branch target if taken
            0x00,
        ]);
        cpu.reset();
        cpu.status &= !FLAG_OVERFLOW;
    ...
    fn test_bne_branches_when_zero_clear() {
        ...
        cpu.load(vec![
            ...
            0xd0, 0x02, // BNE skip next LDA when zero clear
            0xa9, 0x00,
            0xa9, 0x77,
            0x00,
        ]);
    ...
    fn test_rts_pops_return_address_and_resumes() {
        ...
        // Program structure:
        // 0x8000: JSR $8005 -> pushes return address (0x8002) then jumps
        // 0x8003: BRK (skipped)
        // 0x8005: LDA #$77 ; load value inside subroutine
        // 0x8007: RTS      ; return to caller (should resume at 0x8002)
        // 0x8008: LDA #$99 ; only executes after RTS returns
        // 0x800A: BRK
        cpu.load(vec![
            ...
            0xa9, 0x77,       // LDA #$77
            0x60,             // RTS
            0xa9, 0x99,       // caller resumes here
            0x00,
        ]);
        cpu.reset();
        cpu.run();

        // After RTS returns, PC should end at BRK (0x800A) and A should hold 0x99
        assert_eq!(cpu.register_a, 0x99);
